...
class LogicUser {
    ...
    async checkAvailableRoomlogic({ roomType, checkInDate, checkOutDate }, res) {
        try {
            const query = `
            ...
            this.connection.query(query, [roomType, checkInDate, checkOutDate, checkInDate, checkOutDate, checkInDate, checkOutDate], (err, results) => {
                if (err) {
                ...
                res.json({ availableRooms: results[0].availableRooms });
            });
